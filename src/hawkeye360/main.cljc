(ns hawkeye360.main
  "Kotodama WASM entrypoint for the Hawkeye360 clean-room actor (L4) — Clojure port.
  ...
  facts (`hawkeye360.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "hawkeye360")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"hawkeye360.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hawkeye360-compat" :tier tier :entities entities} 200])
...
(defn start! [] :hawkeye360-compat/ready)
